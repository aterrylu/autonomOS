...
**Connection health (ADR-103).** The status-bar dot reads the `/ws/agents` heartbeat (server ping every 5s; stale after 12s → "Reconnecting… last heard Ns ago" → "Disconnected · retrying" at 30s) — not an HTTP poll, which stays only until the socket first opens. A stale socket is ABANDONED and replaced at once; never gate a reconnect on `close()`/`onclose` — on a half-open link the socket sits in CLOSING and `onclose` never fires (measured). Transport stale ⇒ every pane is cut loose and reconnected on recovery (terminal sockets can't detect a half-open link themselves). Per pane, `terminal/connectionWatch.ts` runs an input watchdog: a countable keystroke arms it, any byte back disarms it, 5s of total silence asks `GET /api/agents/:id/io` (input/output AGES) to name the silent side — "Connection lost · reconnecting…" (server never saw the key) vs "Agent not responding · Ns" (it did; the agent printed nothing; enabled only for providers MEASURED to echo while busy). Typing stays live: user keys the socket can't carry are dropped and COUNTED (mouse/focus reports and terminal replies aren't keystrokes). Two server mechanisms stop a recovered link from replaying abandoned keys: the terminal route FENCES input on a superseded socket (`?client=&gen=` query params; `gen` is PAGE-wide — per-pane counters restart and get their healthy socket fenced), and it pings every terminal socket at the WebSocket protocol level and TERMINATES one silent for 7s (below the dashboard's 12s), because the fence alone loses the race when the old socket's retransmit beats the replacement. **Never add a new message type on the terminal socket** — unknown JSON falls through to `pty.write()`, so an older server would type it into the agent; use query params, protocol-level ping/pong, OSC in the downstream, or a REST route. Every socket open replays the scrollback and xterm re-answers the queries in it; replies are dropped until the server's OSC 7777 end-of-replay marker (`?replayMark=1`) is parsed (else `1;2c` lands in the agent's prompt).
