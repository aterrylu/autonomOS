...
**Being replaced (ADR-115, per-runtime):** each CLI's permission options in its OWN canonical vocabulary live in `core/src/types/runtimePermissions.ts` (`RUNTIME_PERMISSIONS` — values, the CLI's own descriptions, caveats, deliberately-not-offered values), and `server/runtimeProbe.ts` checks them against the INSTALLED CLI without starting a session (results on `GET /api/providers`, with real versions). The phase that lands the table does not change spawning — argv still comes from the shared enum below until the migration PR. Never add a user-facing label that isn't the CLI's own value.
