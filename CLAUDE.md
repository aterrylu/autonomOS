...
1. **Live-stream coalescing** (`routes/terminal.ts` `makeStreamForwarder`, #253/#260): Ink emits many tiny chunks per repaint; per-connection buffering with a 5ms **trailing-edge** window (every chunk waits ≤5ms for siblings, so a multi-chunk repaint always ships as ONE frame — unsynchronized TUIs like gemini-cli's Ink emit no DECSET 2026, and the old leading-edge flush shipped their erase-half alone, painting every repaint torn) collapses bursts ~570×. Cost: ≤5ms echo latency after idle, below one display frame. `AUTONOMOS_WS_COALESCE=0` restores per-chunk sends; `AUTONOMOS_WS_COALESCE_LEADING=1` restores leading-edge for ablation.
2. **Reconnect-replay coalescing** (`buildReplayFrames`, ADR-072): on WS connect the server replays the session's whole scrollback buffer (up to 1MB). That replay is an already-complete sequence — time plays no role — so it's a pure fold into ~64KB frames (~16 frames instead of ~19k). Separate flag (`AUTONOMOS_WS_REPLAY_COALESCE=0`) because "today's baseline" for ablation is live-coalescing ON + replay batching OFF.
3. **Keep-alive terminal cache** (`dashboard/src/terminal/liveTerminals.ts`, ADR-072): one live terminal per session — xterm instance, scrollback, WebSocket, reconnect loop — owned by a module cache, NOT by the React mount. Dockview still tears down panels on every switch (ADR-047); `useTerminal` just borrows the cached terminal and reparents its host `<div>`, so the remount is cheap and a switch-back re-streams **nothing** (measured: 37,075 frames / ~1s → 0 frames / ~44ms on loopback; 0 bytes cross the network, so throttling cannot re-introduce the re-stream — ~320ms settled on Slow-3G vs the 85s a cold re-stream takes there). LRU cap 8, attached panes never evicted. A pane whose viewport is parked in the scrollback (trackpad flick, Shift+PageUp — xterm-native scrolling; Codex captures no mouse) shows a "↓ Jump to latest" pill (visible on unfocused panes too); typing also re-follows via xterm's default `scrollOnUserInput`.

Invariants the cache makes load-bearing (each pinned by a dom test; breaking one resurrects a shipped bug): **reset-before-replay** — the server replays the FULL buffer on every connect, so a REconnect must `terminal.reset()` first or duplicate history accumulates permanently; **superseded-socket guards** — every WS handler no-ops unless its socket is still current, else a stale `onclose` re-arms a reconnect that kills the healthy socket in a ~1s churn loop; **deferred session-end** — 4010/4004 frees the cache slot immediately but keeps the final output on screen until the pane departs (and `connect()` refuses `ended` entries); **`detach(container)` is ownership-checked** so a stale mount's cleanup can't blank a newer mount. Do NOT move terminal teardown back into the React effect — that teardown was the original bug. **Never force a repaint with a fake resize from focus/visibility paths** (ADR-087 — `nudgeResize` survives ONLY on reconnect, its #16 birthplace, where follow-state is freshly pinned): ratatui (codex) answers every SIGWINCH with a full rebuild including `\e[3J`, which wipes xterm's scrollback and strands a parked viewport at the top; an ED3/DECSED3 parser hook re-pins a parked viewport to the tail when an app genuinely wipes.

**Connection health (ADR-113).** While you TYPE, per-keystroke acks are the signal: after the server advertises `input-ack` (on the OSC 7777 end-of-replay marker), the pane sends keystrokes as BINARY `[0x01][seq][sentAtMs][bytes]` frames and the server acks `[0x02][seq]` after `pty.write` — unacked at 1s → "Not reaching server… · N keystrokes waiting", 3s → give up + reconnect (keys counted *not sent*; the server drops frames older than 2.5s so they can never land late); acked-but-unechoed at 2s → subtle "Waiting for agent…", 5s → "Agent not responding" (measured providers only; suppressed at `needs_input`). Text frames stay terminal bytes; binary is the negotiated control plane — never send it to a server that didn't advertise. While IDLE, the status-bar dot reads the `/ws/agents` heartbeat (2s ping, stale 5s → "Reconnecting… last heard Ns ago", 20s → "Disconnected · retrying"); a stale socket is ABANDONED and replaced at once — never gate a reconnect on `close()`/`onclose` (on a half-open link it never fires). The server also pings every terminal socket at the WS protocol level (1s) and terminates one silent for 4s, and fences input on superseded sockets (`?client=&gen=`, gen PAGE-wide). **Never add a new message type on the terminal socket's text channel** — unknown JSON falls through to `pty.write()`, so an older server would type it into the agent; use negotiated binary frames, query params, protocol ping/pong, a downstream OSC, or a REST route. Replay: xterm re-answers the queries in the replayed scrollback; replies are dropped until the OSC 7777 marker is parsed (else `1;2c` lands in the agent's prompt).

**Perf-harness mode:** `AUTONOMOS_PERF=1` mounts `/api/perf` (synthetic FakePty sessions + deterministic ink-bursts) and drops auth on the PUBLIC listener — it refuses to engage unless the bind host is loopback, never touches the internal socket's auth, and is stripped from spawned agents' env. Run `./perf/run-l2.sh terminal-switch` (isolated ports + config dir; add `PERF_THROTTLE=1` for the Slow-3G proof) or `bun run perf:l1-replay` for the transport-only ablation.
...
**Accepted messages are recorded for the Org Chart** (ADR-108): `gateway/messageLog.ts` `recordAcceptedMessage` runs at each accept point (never on a failure path) and broadcasts a `message.routed` delta carrying only a sanitized, 60-character one-line preview. Full text (capped at 400) stays in a per-agent in-memory ring buffer read via `GET /api/agents/:id/messages`, and is never pushed.
...
Two gotchas worth knowing. First, **observed on codex 0.144.6** (NOT on 0.154, where a prompt-less agent gets a thread within seconds): an agent spawned *without* a starting prompt showed no thread for minutes — so inbound to it queued indefinitely. On 0.154 the thread exists but codex doesn't SAVE it (rollout written lazily on the first turn), which is why the resume path has a thread pre-flight (ADR-104). Second, the queue is shifted on the `turn/start` **reply**, not on send, so a teardown racing a reply can log a false `DROPPING` — deliberate, because shifting earlier would reopen the #287 silent-drop class.
...
### Env Presets — model overrides (`~/.autonomos/env-presets/`, ADR-067)
Named, reusable sets of env vars applied to an agent at spawn to override its model backend — the motivating case is running Kimi (Moonshot) through the **real Claude Code binary** by injecting `ANTHROPIC_BASE_URL`/`ANTHROPIC_MODEL` + a secret `ANTHROPIC_AUTH_TOKEN` (Moonshot's Anthropic-compatible endpoint). No new provider — the override is entirely env, so hooks/MCP/statusline/permission modes all keep working. One 0600 JSON per preset (`envPresets.ts`, mirrors `schedules.ts`).

**The credential boundary is asymmetric and load-bearing.** A preset splits its vars: `env` (non-secret, plaintext), `secretKeys` (declared secret NAMES, agent-set), `secrets` (VALUES). **Read is a hard wall** — `maskEnvPreset` redacts every secret value on every REST GET and MCP read; `getEnvPresetRaw` (unmasked) exists only for spawn injection and is wired to no response. **Write is a surface convention** — the MCP tools (`create_env_preset`/`update_env_preset`) and the channel-server dispatch omit/strip `secrets`, so an agent configures everything *except* the key; the REST route (dashboard) accepts secrets, so a human keys it in the Presets tab. Tool descriptions tell agents not to solicit tokens in chat. Honest caveat (see ADR-067, narrowed by review): the guarantee is *only* that the management API never returns a full key (last-4 mask) and no MCP/channel path writes one — NOT "the key is unreadable." A preset is a capability to USE a credential, so a spawned child has the real key in its env (echo + relay leaks it) and an agent can repoint `ANTHROPIC_BASE_URL` to exfiltrate it on the next spawn. Presets assume a **trusted agent fleet**, not a malicious one.
...
Each agent record (`$configDir/agents/<id>.json`) carries a `managerId`; the hierarchy is derived at query time by ONE algorithm in core (`buildTreeFromRecords` / `buildAgentTreeNodes`, `core/src/tree.ts`) used by `GET /api/agents/tree`, MCP `get_org_chart`, and the dashboard push bridge. "Live" and "manager" also have ONE definition there (`isLiveAgent` = `status === "running"`, `hierarchyOf`), shared by the tree and the statusline's `GET /api/agents/:id/self`, so the two can't disagree. Its default filters exited agents and promotes their reports to roots — right for the sidebar and MCP, wrong for the chart. Set at runtime via `set_manager` (MCP) or `POST /api/agents/:id/manager` (cycles → 409).

**The Org Chart tab** (`HierarchyPanel.tsx` + `components/orgchart/`, ADR-112) reads its own `orgTreePoll` (`?includeExited=true`, push-fed via the same derive) and prunes in the client (`pruneExited`): an exited agent is drawn as a dashed ghost only while its subtree still holds a running agent, so a crashed manager keeps its team; other exited agents hide behind "Show N exited". Layout is a tidy tree (`layout.ts`: teams side by side, reportless roots on an "Unassigned" shelf). Every color comes from `orgChartTokens(page)` + `statusLabelStyle` — never add a literal color to a card. A click SELECTS (never navigates away): the chain stays lit, and a docked inspector (`OrgInspector`) shows the agent with explicit actions. Double-click / Enter / the inspector's button opens the terminal; arrows walk the chart. The inspector STICKS: an empty-canvas click does not close it, another card switches it, and it closes on its ×, Esc (escape stack, only while the chart is engaged), or when the pane is hidden (`HierarchyPanel visible`, forwarded from dockview). Cards sharing a name carry a short `#id` hint. **Cards + layout** (ADR-118): a lead's LEAF reports stack in a column off a spine (`layoutOrg`'s `stacked` map + `edgePath()` — the one connector for edges AND message envelopes; reports that lead teams, or are folded, stay side by side). The Balanced card = an id-seeded `Identicon` + the unaltered provider/status badge, an unread pill, a pulsing dot, the live action (Codex: "Working"), time in state, and a 24h strip; the strip/time come from ONE batched request (`useFleetActivity` → `GET /api/agents/analytics`), never one per card, with window/colors shared with the inspector (`orgchart/activityStrip.ts`). **Canvas** (ADR-117, `orgchart/useCanvasView.ts` + pure math in `orgchart/viewport.ts`): drag EMPTY canvas to pan (< 4 px = a click, which does nothing); a drag starting on a card/button/bubble never pans — card drags are reserved for PR 4 (reassign); scroll pans, pinch / ⌘ + scroll zooms at the pointer (30–200%); opens fitted with a 60% floor; Fit / 0 / + / − (chart-local keys). The view `{x,y,k}` lives OUTSIDE React state — the stage transform is written to the DOM and only subscribers (zoom %, the map) re-render; never move it into state (a pan would re-render every card per frame). The bottom-right `Minimap` shows only when the chart doesn't fit. Bubbles counter-scale via the stage's `--org-inv-k` and the independent `scale` property (composes with the unfurl `transform`). Right-click opens the shared `AgentContextMenu`, **portaled to `<body>`** because dockview's `.dv-render-overlay` (transform + contain) re-anchors `position: fixed` to the pane. Cards expose `data-org-card` / `data-org-status`, and the stage is `data-org-stage` (the hero script fits it by that hook). **Team rollups + collapse** (`orgchart/teams.ts`): each lead shows chips counting its whole subtree AS DRAWN (after pruning, so hidden exited records never inflate "N exited"), computed BEFORE folding so a collapsed lead still says who needs you. Folded teams persist per browser under the `autonomos.orgchart.collapsed` localStorage key; a folded root stays in the team row via `layoutOrg`'s `isTeam`. **Message flow** (`orgchart/MessageLayer.tsx`, ADR-108): `message.routed` frames arrive on a transient `agentsSocket.onMessageRouted` channel (not the snapshot), become envelopes plus speech bubbles (text only, coalesced per recipient, at most 3 in flight, a timer backstop because rAF pauses in hidden tabs), and the inspector's Communication section reads the on-demand log. The mode (Animated / Quiet / Off) is remembered per browser under `autonomos.orgchart.messages`. **Inspector analytics** (`orgchart/Inspector.tsx` + `agents/analytics.ts`, ADR-110): `GET /api/agents/:id/analytics` returns counts since the server started. They come from existing taps: `emitStatusDelta` (time in state, turns, waits, 24h strip), hook ingest (tools and failures), and the spawn site/`markExited`/PTY exit (restarts, crashes, exit code). Starts are counted in `spawnAgent`, not `markRunning`. An exit closes the state as `stopped`, so a killed agent is never "waiting now". `support` flags per runtime make the panel show "n/a for Codex" rather than zeros. Usage (context/cost/model) comes only through TerminalRender's shared statusline mechanism, never a second path. The cards' status + 24h strip for the WHOLE fleet come from ONE request, `GET /api/agents/analytics` (`?ids=` optional; registered before `/:id`), built on the same `activityStrip()` as the inspector and down-sampled to `CARD_STRIP_MAX_SEGMENTS` (48; a bucket with any needs-input shows needs-input). Never fetch analytics per card.
...
All app-level chords live in the registry at `packages/dashboard/src/shortcuts/registry.ts` (ADR-063) — one table consumed by the window capture-phase dispatcher (`useShortcuts`) AND xterm's key handler (`isReservedChord` consult). Never add an ad-hoc keydown listener for a global shortcut; add a registry entry (its unit test enforces chord uniqueness and the browser-reserved / terminal-sacred deny-lists). Current set: mod+1-9 switch to the Nth SIDEBAR agent row, mod+↑/↓ previous/next agent (clamped, ↑/↓ hint chips on the active agent's neighbors during hold) (ADR-066 — rendered order, published by Sidebar as `sidebarRowOrder`; NOT open-pane positions), mod+B sidebar, mod+/ help overlay, mod+K agent quick-switcher (fuzzy type-ahead; terminal-clear moved to mod+shift+K, ADR-071). Holding mod ~350ms shows each agent row's digit badge (`useModKeyHold` + `sidebarRowOrder.ts` — badges and shortcuts read the same published list, so the hint cannot lie). Escape dismissal is the registry's `ui.dismiss` entry backed by the LIFO escape stack (`shortcuts/escapeStack.ts`, ADR-065) — dismissible UI pushes a closer while mounted-open; never add a document-level Escape listener (bubble-phase listeners are dead under terminal focus). ADR-065 also freed the legacy ctrl+d/w/b terminal swallow; `mod+w` stays declined deliberately (browser-owned close-tab). The key-capture boundary (which chords the app steals from a focused terminal) is owned by Shortcuts@autonomOS.
