...
1. **Live-stream coalescing** (`routes/terminal.ts` `makeStreamForwarder`, #253/#260): Ink emits many tiny chunks per repaint; per-connection buffering with an 8ms **leading-edge** window (first chunk after idle flushes immediately → zero added echo latency) collapses bursts ~570×. `AUTONOMOS_WS_COALESCE=0` restores per-chunk sends.
