...
**Per-runtime canonical permissions (ADR-115) — the SOURCE of truth now.** An agent's `permission` is `{runtime, values}` in its CLI's OWN vocabulary (`core/src/types/runtimePermissions.ts`: `RUNTIME_PERMISSIONS` table + `completePermission`/`parseRuntimePermission`/`formatPermission`/`permissionFromLegacyMode`); providers build argv from it via `effectivePermission()` (`providers/shared.ts`). `permissionMode` below survives only as the LEGACY projection (`legacyModeFor`) for readers not yet migrated, and as accepted-but-DEPRECATED API input mapped to exactly what it always ran. Records migrate at load (`store.ts`; Codex auto/plan get a one-time notice). Spawn order: explicit > record (resume) > template `permissions` > operator's `runtimeDefaults` (server settings) > built-in default. Two measured exceptions (ADR-119): Claude `manual` is spawned WITHOUT a flag (the flag leaked processes past teardown in an A/B), and Codex `collaboration_mode` is per-turn so non-default values are REFUSED (`perTurn`). `server/runtimeProbe.ts` checks the table against the INSTALLED CLI without starting a session. Never add a user-facing label that isn't the CLI's own value.
