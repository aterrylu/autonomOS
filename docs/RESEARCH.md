...
- [x] **Agent memory systems** — Should autonomOS add a memory system (per-agent, fleet-shared, or both)? Hands-on eval of mem0, Letta, Zep/Graphiti, cognee, Supermemory, Honcho, Memori, framework modules, markdown+MCP prior art, and the three CLIs' native memory; spikes over Terry's real 134-file Claude Code memory corpus. Recommendation: custom markdown-first store in `$configDir/memory/` + 3 MCP tools + spawn-time digest; Phase 0 = bridge CC's existing memory to Codex/Gemini. Full analysis: [`docs/research/agent-memory/`](research/agent-memory/)
...
**Key findings:**
- **Claude Code's auto-memory is already the fleet memory — for Claude Code agents only.** Terry's repo dir holds 134 fact files written by 44 sessions; every CC agent in the repo shares it (keyed by git repo, worktrees included). Another agent appended to the index mid-evaluation. Codex and Gemini agents cannot read a byte of it. That is the actual gap.
- **Retrieval is not the hard part.** Over the real corpus, plain FTS5 hit the right file in the top 3 on 8/8 realistic queries (37 ms); local embeddings only improved top-1 (6/8 → 8/8). A v1 needs no model dependency.
- **LLM extraction is the expensive, fragile part — and it is the only thing memory platforms add.** mem0 fully local (Ollama) paraphrased rules into third person, duplicated one, and stored a contradiction as a new fact. Verbatim agent-written memories beat extraction for rules whose wording matters.
- **The platform OSS tiers are a moving floor:** mem0 removed graph memory from OSS (v2.0.0), Zep retired Community Edition, Supermemory's self-host has a 10k-document cap, Letta's Python server is EOL (confirmed: the Docker image exits with an EOL banner) and shared memory is cloud-only, Memori's extraction is hosted even in BYODB mode. cognee is the only platform that is fully local with an official local MCP server.
- **Custom path is cheap because the seams exist:** `buildSystemPrompt()` reaches all three providers (the only path to Gemini, which does not launch the channel server today); the channel-server MCP switch + `ToolDef`/`readOnlyHint` + gateway per-agent token give attributable `remember`/`recall`; `handoffQueue.ts` is the persistence recipe; `autoMemoryDirectory` is one key in the inline `--settings` payload.
- **Native CLI memory:** Codex "Memories" (off by default, background-generated, user-keyed, don't hand-edit); Gemini removed `save_memory` in 0.40.0 and Google retired Gemini CLI for consumer tiers (2026-06-18, Antigravity CLI); none keys memory by agent identity.

**Recommendation:** Custom, markdown-first store under `$configDir/memory/{fleet,agents/<name>}/` with a rebuildable SQLite FTS5 index, three MCP tools (`recall` read-only, `remember` gateway-routed, `forget`), a spawn-time digest via `buildSystemPrompt`, and a dashboard Memory pane. **Phase 0 (S):** read-only Memory pane over CC's existing dir + inject its `MEMORY.md` index into Codex/Gemini prompts. **Phase 1 (M):** the store. **Phase 2 (M):** capture nudges on PreCompact/SessionEnd, hygiene, optional local embeddings. Eight questions for Terry are in the README.
...
**Addendum (2026-09-12):** Terry locked scope/storage/writes and replaced the Phase 0 bridge with a coexistence question. Verified that an inline `--settings` `autoMemoryDirectory` redirects a spawned CC session's memory read+write; recommended option A1 — Claude Code's native memory becomes a *client* of the autonomOS store (`projects/<project>/` ≈ CC's per-repo dir), complete without CC, reversible by dropping one key. See `coexistence.md`. **Addendum 2 (2026-09-12):** Terry reframed toward adopting an EXTERNAL system. Re-ranked the field under seven constraints (provider-neutral via our own channel-server MCP, self-host, multi-machine, longevity, verbatim mode, scoping, attribution) and booted the top candidates: mem0 `infer=False` stores verbatim with no LLM call; Honcho self-hosts in one compose, stores verbatim with native observer/observed attribution, recalls "3100" by the number, but pair-scopes every query; cognee cannot search before an LLM cognify. Recommendation under that frame: **mem0 OSS self-hosted behind our channel server, verbatim by default, mem0 Platform as the hosted fallback; Honcho if the reasoning layer is wanted.** See `external.md`. **Phase 1 spec (2026-09-12):** Terry locked mem0 OSS + vendor-image embeddings + leave CC memory alone, and reframed memory as a product feature for other autonomOS users. `phase-1-spec.md` specifies: off by default with a `NullBackend` as the production default; three modes (managed via a pinned docker compose, external URL, mem0 Platform); secrets on the env-preset pattern; three tools hidden from `ListTools` when off; digest per spawn with an on-disk stale cache for the unavailable case; `autonomos memory` CLI verbs; test strategy without Docker in CI; eight product-frame hazards (Docker as optional dependency, image boot requirements, OpenAI/Gemini-only embeddings, digest pinning, Gemini digest-only).

Full analysis: [`docs/research/agent-memory/`](research/agent-memory/) — README (evaluation + phased proposal + questionnaire), phase-1-spec (product-frame Phase 1 design), external (adopt-an-external-system option space + rec), coexistence (CC-native relationship options + rec), landscape (profiles + sources), hands-on (spike data).
