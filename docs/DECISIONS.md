...
## ADR-054: Auth on /mcp + loopback bind by default (closes an unauthenticated RCE)
- **Date:** 2026-07-17 — **Decided by:** Human (Terry: "can we always require auth? hard requirement", and confirmed he browses to `forge:3100` directly), investigated + implemented by AppSecurityAudit@autonomOS under TeamLead@autonomOS lead
...
- **Decision:** (a) Mount `requireAuth` on `/mcp`, checked at the transport boundary so an unauthenticated caller cannot complete `initialize` and never obtains the session id later calls need. (b) Default the bind to `127.0.0.1`, with `--host` / `AUTONOMOS_HOST` as an explicit opt-in, threaded through `install-service` → `install-prod-service.sh` → `Makefile` (`BIND_HOST`) so a deliberately-exposed box keeps its bind. (c) An exposed bind logs a startup warning that **names the still-unauthenticated routes** rather than claiming blanket coverage.
- **Rationale:** Terry's "always require auth" ruling is the load-bearing simplification. It makes auth the boundary and the bind defense-in-depth, which deletes the entire *category* rather than patching instances: no localhost-trust, no peer-address gate, no exemption to get subtly wrong. It costs nothing because `auth.ts` has always generated + persisted a random 256-bit token on first start (`randomBytes(32)`, `0600`) — there is no tokenless state to design for. Loopback-by-default additionally makes the whole perimeter unreachable for the majority who run the dashboard on their own machine.
- **Alternatives considered:**
...
- **Breaking change:** a server reached over the network goes loopback-only on upgrade unless `AUTONOMOS_HOST=0.0.0.0` is set in that box's `.env` (not rsynced, so it persists per box) or `BIND_HOST=0.0.0.0` is passed to `make deploy`. `AUTONOMOS_HOST` is the recommended form: the service wrapper runs `tsx --env-file=<repo>/.env`, so it survives `install-service --force`, whereas a service-file-baked `--host` is dropped by a reinstall that omits it.
- **Note:** Refines ADR-041's loopback-trust reasoning by removing the trust rather than gating it. Composes with ADR-050/052 (launchd/systemd-user supervision — `--host` threads through the same installer path).
