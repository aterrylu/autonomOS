...
- **Decision:** Remove the feature entirely in one PR rather than deprecating in phases.
  1. **Delete the gating** — `DEFAULT_CAPABILITIES`, `CAPABILITY_GATED_TOOLS`, and `filterToolsByCapabilities` in `mcp/tools.ts`; the `AUTONOMOS_CAPABILITIES` env read, the list filter, and the call-time rejection in `channel-server/index.ts`; the env injection in the Claude Code and Codex providers; `ResolvedSpawnOptions.capabilities` and its resolution in `agents/runtime.ts`; the `capabilities` parameter on `create_template` (both MCP transports) and on `POST /api/templates`.
