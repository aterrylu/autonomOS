...
- **Decision.** `.github/workflows/changeset-check.yml` now **fails (exit 1)** when a PR (a) touches production source under `packages/*/src` — excluding test files (`__tests__/` directories and `*.test.*`, which this repo keeps inside `src/`) — AND (b) `bun changeset status --since=origin/<base>` reports uncovered changed packages. Docs/CI/scripts/test-only PRs never reach the status check and stay frictionless. `bun run changeset --empty` remains the explicit opt-out for source changes with no version impact. The `changeset-release/main` bot PR skip is preserved.
- **Rationale.** Two signals, each doing what it's good at: the git-diff path filter decides *whether this PR is substantive* (changesets' own changed-package detection is stricter than we want — any file in a package counts, which would tax test/config-only PRs), and `changeset status` decides *whether a changeset covers it* (verified empirically: exit 1 for uncovered changes, exit 0 with a real changeset, and — the load-bearing case — **exit 0 with an `--empty` changeset**, so the escape hatch needs no extra logic). Fail-closed: an infrastructure error in either step fails the job visibly rather than waving the PR through.
- **Alternatives considered and rejected:**
  - *Keep it advisory, rely on review discipline.* That was the status quo; it failed 5 times in one release cycle.
  - *Gate on `changeset status` alone (no path filter).* Blocks test-only and config-only PRs — friction the escape hatch would absorb, but at the cost of training people to reflexively add `--empty` changesets, which erodes the signal.
  - *Gate on raw `.changeset/*.md` file presence in the diff.* Duplicates coverage logic changesets already implements, and misses the "changeset present but for the wrong package" case that `changeset status` catches.
