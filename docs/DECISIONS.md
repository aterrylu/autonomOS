...
- **Decision:**
  ...
  2. **One resolution point.** `params.permissionMode ?? DEFAULT_PERMISSION_MODE` is computed **once** in `spawnAgent`, and the record, the argv, and the write-back all read that one value. Previously the record and the argv defaulted independently and agreed only because both happened to name the same constant.
  3. **Rename the value `"default"` → `"ask"`,** matching the label users already see and freeing the ordinary word for policy prose. `PERMISSION_MODES` is now the sole declaration and `PermissionMode` is derived from it, so a fifth mode cannot drift out of the schemas.
...
- **Verification:** mutation-tested, not merely green. Reverting the `markRunning` write turns 2 persistence tests red; removing the legacy alias turns 3 migration tests red. The migration tests deliberately pair a legacy spelling with a conflicting `autonomousMode` so they distinguish "the alias fired" from "an unrecognized value fell back to the same answer" — without that, they would pass with the migration deleted. Full suite: 765 server + 272 dashboard.
