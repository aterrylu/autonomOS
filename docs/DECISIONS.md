...
- **Decision.** Only the timestamp span changes; the row, name, status icon, and repo·branch line are untouched. Age buckets and treatment ("Balanced wide" ramp): **fresh** (<1h) → the theme's **foreground / text color** at full opacity (void `#d4d4d4`); **recent** (1–24h) → neutral `statusFg`, full opacity; **stale** (1–7d) → `statusFg` at **72%**; **ancient** (≥7d) → `statusFg` at **52%**. The lower three thresholds coincide with `formatAge`'s own unit boundaries (m→h→d); the 7d stale→ancient step is a recency threshold, not a unit change. Opacity (not a pre-blended color) carries the fade so it recedes toward whatever is behind the text — page background or an active-row highlight — and adapts across themes for free. Buckets are computed inline from the same `lastActive` the text renders from, on the sidebar's existing ~5s render pass — **no new timer**. Logic lives in a pure, tested `recency.ts` (`recencyBucket` + `recencyTimestampStyle`) with zero embedded palette.
...
- **Test evidence.** `recency.test.ts` pins exact bucket boundaries, the opacity/color mapping (72%/52%), the theme-`fg` fresh color, and degenerate timestamps (0/negative/future/NaN → full opacity); `Sidebar.recency.dom.test.tsx` pins the wiring end-to-end (ancient row → 0.52, fresh → `fg` `rgb(212,212,212)` at full opacity, name NOT faded, missing-timestamp "unknown" → full opacity). Both the guard fix and the ramp are mutation-verified RED first. Rendering visually verified across void/midnight/daylight and on a live proxied fleet.
