...
> **Update (2026-07-25): SUPERSEDED by ADR-059.** The feature was removed entirely — all three layers plus `GET /api/files/read` and `WS /ws/files/watch`. It had silently regressed (the dockview default in #263 made `openPreview` tear down the whole dock; the hierarchy-sidebar default in #249 left previews with no row) and carried no test coverage. See ADR-059 for the audit, the removal, and the persisted-state handling.
...
- **Backward-compat (persisted state):** `previewPanes` was persisted via `partialize`, and `activePane` could hold `{type:"preview"}`. There is no zustand `version`/`migrate` hook — all schema handling lives in the hand-rolled `merge` — and a version bump was rejected because it would reset every user's entire layout to fix one nullable field. Instead the stale state self-heals:
  - `activePane: {type:"preview"}` is now **rejected by `isValidActivePane`** and degrades to `activePane: null` (the friendly empty state). Without this it would restore cleanly, reach `PaneContent`, find no `"preview"` case, and fall through to `default: return null` — a **silently blank pane**, the exact silent-failure class this repo guards against. A regression test now asserts the rejection.
  ...
  - Leftover `preview:*` keys in `pinnedOrder`/`unpinnedOrder` are swept by `fetchSessions`' existing prune once `keepKey`'s `startsWith("preview:")` escape hatch is removed — the prune loop becomes the migration, at no extra cost.
  - Preview ids inside `dvWorkspaces[*].paneIds` are reclassified as dead sessions and removed by `reconcileDeadWorkspaces`.
  - `/preview?file=…` URLs keep resolving (the production SPA fallback) and simply render the normal dashboard; a hard 404 was deliberately not added.
