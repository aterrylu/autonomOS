...
## ADR-072 — Settle-gated prompt-delivery receipt with retractable give-up
...
- **Source:** notifications/warnings audit (docs/research/notifications-warnings-audit.md) + this session's self-referential evidence (the audit agent's own panel entries and double-received brief); builds on ADR-057 (capability gate) and ADR-071 (retractable warnings).
