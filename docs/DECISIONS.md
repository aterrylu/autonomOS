...
- **Alternatives considered:** archive flag (declined, above); rename now (deferred, above); an undo-toast for delete (rejected — no deferred-delete plumbing, and a delete is irreversible); a flyout submenu for Set manager (rejected in favor of an in-place drill-in — no second-level positioning, fully keyboard-navigable in the same container); wiring only running rows (rejected — the clutter-delete lives on the exited Projects surface, the feature's headline value).
- **Source:** Terry via TeamLead relay 2026-08-28 ("let's get the rest of the design shipped"); design artifact https://claude.ai/code/artifact/accf2bfd-6690-45c6-975e-07771b12e272.
