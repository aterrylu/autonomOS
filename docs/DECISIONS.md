...
- **Server-side addition required:** `autonomos-server` must write `~/.autonomos/server-state.json` (`{ port, pid, version, startedAt }`) at boot so the desktop can discover the running daemon. ~10 LOC; ships in Phase 1B.2 alongside the desktop work.
