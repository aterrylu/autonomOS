...
- **Test evidence.** Ablation harnesses above; 9 dom tests pin the cache lifecycle (detach does NOT dispose/close — the core fix asserted in the negative; LRU evicts first-detached only, never attached; 4010 disposes + routes the UI away; network-blip close keeps the entry and reconnects; failed backend caches nothing); 6 unit tests pin `buildReplayFrames` (byte-identity, order, oversized chunk, empty buffer, flag-off passthrough). Server suite 812/812, dashboard 372/372 green.
