...
- **`onComplete` is deprecated rather than rehomed — a near-miss worth recording.** It was gated on `target === "isolated"`, which made it dead code the moment that went away, and the obvious tidy is to un-gate it so it works for the surviving target. That would have been wrong, and the existing test asserting "does NOT deliver onComplete for agent mode" (no rationale attached) was the only thing pointing at it. For `isolated`, a run completed when the child exited, so "completed: success" meant the task had RUN. For `agent:`, `executeAgentSend` reports success as soon as `routeMessage` returns no error — **the agent has not started**. Un-gating would have shipped a notification announcing a completion that had not happened. Reporting real agent-side completion needs the agent to signal back; that is a different feature. **Generalizable: before rehoming a feature onto surviving semantics, check the semantics actually match — an arbitrary-looking gate may be load-bearing.**
- **Alternatives considered:**
