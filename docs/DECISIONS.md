...
**Source:** Claude Code session (plugin system feature)

**Context:** The dashboard needs extensibility for features like usage monitoring, provider status, and future tools. Rather than adding one-off components, we need a plugin system that's modular and easy to extend.

**Decision:** Static plugin registry with a VSCode-like bottom status bar. Each plugin is a self-contained module that declares status bar items and optional panels. Plugins are imported at build time and registered in a simple array. First plugin: Claude Usage (aggregates token usage from JSONL session files with mtime-based caching).

**Plugin interface:**
- `DashboardPlugin` — declares id, name, statusBarItems[], panels[]
- `StatusBarItem` — id, align (left/right), priority, component (React ComponentType)
- Server-side routes are mounted explicitly (no auto-discovery)

**Rationale:**
- Static registry avoids dynamic loading complexity — personal tool with <10 plugins
- Self-contained components manage their own data fetching — StatusBar just renders them
- Plugin system is the right abstraction for provider-specific features (usage, status)
- VSCode's status bar is proven UX for compact, always-visible information

**Alternatives:**
- **Dynamic plugin loading** — Overkill for a personal tool, adds lazy loading / sandboxing complexity
- **Zustand slices per plugin** — Couples plugins to the store, makes them harder to add/remove
- **No plugin system** — Would lead to ad-hoc components scattered across the codebase
