...
import { DRAG_TYPE, encodeDragData, useDragContext } from "./DragContext";
 ...
 * Tabs are draggable — drop them on another container to move between panes.
 */
export function TabBar({ leafId, tabs, activeTabIndex }: TabBarProps) {
  ...
  const { isDragging, dragData, startDrag, endDrag } = useDragContext();
  ...
  return (
    ...
    >
      {tabs.map((tab, i) => {
        ...
        const isBeingDragged = isDragging && dragData?.pane.id === tab.pane.id;
        return (
          ...
          <div
            ...
            draggable
            onMouseDown={(e) => {
              // Left-click must propagate for drag initiation
              if (e.button !== 0) e.preventDefault();
            }}
            onDragStart={(e) => handleTabDragStart(e, tab)}
            onDragEnd={() => endDrag()}
            onClick={() => switchTabInLeaf(leafId, i)}
            className={`group/tab flex items-center gap-1.5 px-2 text-[11px] border-r transition-colors min-w-0 ${i === activeTabIndex ? "font-medium" : "hover:brightness-125"}`}
            style={{
              height: TAB_HEIGHT,
