...
- **Review hardening (nox):** (a) the opened gap is now a real drop target — the `<aside>` is the single drop authority (`onDragOver` preventDefaults during an active drag, `onDrop` commits from the drop-target state), so releasing in the gap over the pointerEvents:none ghost commits instead of silently cancelling; (b) flat reorder now commits by KEY, not index, so a fresh agent arriving mid-drag can't shift the landing slot (the same class as the hierarchy fix); (c) auto-scroll is fed from one source and stopped on a geometry-checked `dragleave`, so it can't run away when the drag exits to the dockview area. New tests: a container/gap-drop reproduction (mutation-verified), a mid-drag-arrival commit-by-key case, and a beforeKey-exited fallback.
