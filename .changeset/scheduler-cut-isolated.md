...
`autonomous`, `workingDirectory`, `template` and `onComplete` are accepted and ignored rather than rejected, so existing schedule files and older MCP clients keep working. (`onComplete`'s delivery code is deleted, not merely disabled — left gated on the removed target it would still have fired for schedules already on disk.) An existing `isolated` schedule still loads and stays editable, warns once at startup naming itself, and fails any run with a message pointing at `agent:<name>`. See ADR-062.
