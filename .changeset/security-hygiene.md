...
- **Schedule shape validation on load.** `getSchedule` now rejects a file that isn't a well-formed `Schedule` (a list-wrapped `[]`, a non-object, or one missing a required `name`/`schedule`/`target`/`prompt`/`enabled` field or its `state` object) instead of casting it through — which used to let a malformed file drive cron construction or a gateway send off undefined values, or (for a state-less file) crash `initScheduler()` entirely, since the scheduler dereferences `state` outside its per-schedule try. Mirrors the templates guard. Deprecated/accepted-and-ignored fields are still allowed, so pre-removal schedule files keep loading.
